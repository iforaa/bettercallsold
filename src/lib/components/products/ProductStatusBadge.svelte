<script lang="ts">
	import { getProductStatusDisplay, getProductStatusColor } from '$lib/utils/status';
	...
	const statusDisplay = $derived(getProductStatusDisplay(status));
	const statusColor = $derived(getProductStatusColor(status));
</script>
