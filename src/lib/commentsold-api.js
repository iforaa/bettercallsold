/**
 * CommentSold API Client for SvelteKit
 * Adapted from the original TypeScript implementation
 */
...
export class CommentSoldResponse {
  constructor(products, liveVideo, banner, total) {
...
export class CommentSoldAPI {
  constructor(baseUrl = "https://api.commentsold.com/api/2.0/divas") {
    this.baseUrl = baseUrl;
    this.headers = {
      "User-Agent": "CommentSold-SvelteKit-Sync/1.0",
  ...
  async getProducts(lastPostId, collectionId) {
  ...
  parseProductDetails(productData) {
    return new CommentSoldProduct(productData);
  }

  async getAllProductsFromCollection(collectionId, limit) {
    const allProducts = [];
    let lastPostId;
  ...
  async getProductsWithCollectionInfo(collections, productsPerCollection) {
  ...
  async getLiveSaleDetails(liveSaleId) {
  ...
  async getLiveSalesWithDetails(limit) {
