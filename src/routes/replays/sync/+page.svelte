<script lang="ts">
	...
	async function startSync() {
		...
		addLog(`📋 Settings: ${syncSettings.maxReplays} max replays, include products: ${syncSettings.includeProducts}`, 'info');

		try {
			// Start the sync process
			addLog('📡 Sending sync request to server...', 'info');
			
			const response = await fetch('/api/cs-sync/replays', {
				method: 'POST',
				headers: {
			...
			let progressSteps = [
				{ progress: 10, message: '📡 Connecting to CommentSold API...' },
				{ progress: 25, message: '🎬 Fetching live sales list...' },
				{ progress: 40, message: '📦 Loading replay details...' },
				{ progress: 60, message: '🔍 Processing replay metadata...' },
				{ progress: 75, message: '💾 Saving replays to database...' },
				{ progress: 90, message: '🛍️ Syncing replay products...' },
			...
			showToast(`Successfully synced ${results.imported + results.updated} replays!`, 'success');
		} catch (error) {
...
<svelte:head>
	<title>CommentSold Replays Sync - BetterCallSold</title>
</svelte:head>
	...
	<div class="page-header">
		<div class="header-main">
			<h1>
				...
				CommentSold Replays Sync
			</h1>
		</div>
		<p class="page-description">
			Import live sale replays from CommentSold API into your BetterCallSold database.
		</p>
	</div>
	...
	<div class="page-content">
		...
		<div class="settings-card">
			<div class="card-header">
				<h2>Sync Settings</h2>
				<p>Configure your replay import preferences</p>
			</div>
				...
				<div class="settings-section">
					...
					<div class="settings-grid">
						<div class="setting-item">
							<label for="maxReplays">Maximum Replays</label>
							<input 
								id="maxReplays"
								type="number" 
								bind:value={syncSettings.maxReplays}
								min="1"
								max="100"
								class="setting-input"
							/>
							<span class="setting-help">Limit the number of replays to import (1-100)</span>
						</div>
						
						<div class="setting-item">
							<label for="syncMode">Sync Mode</label>
							<select id="syncMode" bind:value={syncSettings.syncMode} class="setting-select">
								<option value="update">Update existing replays</option>
								<option value="replace">Replace all replays</option>
							</select>
							<span class="setting-help">Choose how to handle existing replays</span>
						</div>
					</div>
				</div>
				...
				<div class="settings-section">
					...
					<div class="checkbox-group">
						<label class="checkbox-item">
							<input 
								type="checkbox" 
								bind:checked={syncSettings.includeProducts}
							/>
							<span class="checkbox-label">Include replay products</span>
							<span class="checkbox-help">Import products featured in each replay</span>
						</label>
					</div>
				</div>
			</div>
			...
			<div class="status-card">
				...
				{#if syncStatus === 'idle'}
					<div class="status-idle">
						...
						<h3>Ready to Sync</h3>
						<p>Configure your settings above and click "Start Sync" to begin importing replays.</p>
						<button 
				...
				{:else if syncStatus === 'completed'}
					<div class="status-completed">
						...
						{#if syncResults}
							<div class="results-summary">
								<div class="result-item">
									<span class="result-label">Replays Imported:</span>
									<span class="result-value">{syncResults.imported}</span>
								</div>
								<div class="result-item">
									<span class="result-label">Replays Updated:</span>
						...
						<div class="action-buttons">
							<button class="btn-primary" onclick={() => goto('/replays')}>
								View Replays
							</button>
