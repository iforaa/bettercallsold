			...
			<div class="page-header-content">
				<div class="page-header-nav">
					...
					<div class="breadcrumb" style="margin-bottom: 0; font-size: var(--font-size-lg); font-weight: var(--font-weight-semibold);">
						...
						<span class="breadcrumb-item current">{currentVariant.title}</span>
					</div>
				</div>
				<div class="page-actions">
					<button class="btn btn-primary" onclick={saveVariant} disabled={saving}>
						{#if saving}
							<span class="loading-spinner"></span>
						{/if}
						{saving ? 'Saving...' : 'Save'}
					...
					<div class="sidebar-section">
							...
							<div class="nav-context-info">
								<div class="nav-context-icon">
									{#if getFirstImage(product)}
										<img src={getFirstImage(product)} alt={product.name} style="width: 100%; height: 100%; object-fit: cover; border-radius: var(--radius-sm);" />
									{:else}
										📦
									{/if}
								</div>
								<div class="nav-context-details">
									<h3 class="nav-context-title">{product.name}</h3>
						...
						<nav class="nav-list">
							{#each filteredVariants as variant}
								<div class="nav-item">
									<button 
										class="nav-link {variant.id === currentVariant.id ? 'active' : ''}"
										onclick={() => selectVariant(variant)}
									>
										<span class="nav-link-icon">📦</span>
										<span class="nav-link-text">{variant.title}</span>
									</button>
								</div>
							{/each}
						</nav>
				...
				<div class="content-main">
						...
						<div class="media-upload-area">
							<div class="media-upload-content">
								<div class="media-upload-actions">
									<button class="btn-secondary">+ Add image</button>
								</div>
								<p class="media-upload-hint">or drop an image to upload</p>
							</div>
						</div>
					</div>
					...
					<div class="content-section content-section-padded">
						...
						<div class="form-field">
							<div class="form-checkbox-field">
								<input type="checkbox" id="charge-tax" checked class="form-checkbox" />
								<label for="charge-tax" class="form-checkbox-label">Charge tax on this variant</label>
							</div>
						</div>
					...
					<div class="content-section">
						<div class="content-header content-header-clean">
							...
							<button class="btn-link">Adjustment history</button>
						</div>
						<div class="content-body">
							...
							<div class="form-field">
								<div class="form-checkbox-field">
									<input type="checkbox" id="track-quantity" bind:checked={trackQuantity} class="form-checkbox" />
									<label for="track-quantity" class="form-checkbox-label">Track quantity</label>
								</div>
							</div>
							
							<div class="form-field">
								<div class="form-checkbox-field">
									<input type="checkbox" id="continue-selling" bind:checked={continueSelling} class="form-checkbox" />
									<label for="continue-selling" class="form-checkbox-label">Continue selling when out of stock</label>
								</div>
							</div>
						
							<div class="content-section content-section-compact">
								<div class="content-header content-header-clean">
									<h4 class="content-title">Quantity</h4>
									<button class="btn-link">Edit locations</button>
								</div>
								...
								<div class="table-container">
									<table class="table table-compact">
										<thead>
											<tr>
												<th class="table-cell-main">Location</th>
										...
										<tbody>
											<tr class="table-row">
												<td class="table-cell-main">
													<span class="table-cell-text">Shop location</span>
												</td>
					...
					<div class="content-section content-section-padded">
						...
						<div class="form-field">
							<div class="form-checkbox-field">
								<input type="checkbox" id="physical-product" bind:checked={requiresShipping} class="form-checkbox" />
								<label for="physical-product" class="form-checkbox-label">This is a physical product</label>
							</div>
						</div>
						...
						<div class="form-field">
							...
							<div class="form-input-group">
								<input 
									...
									step="0.1"
								/>
								<select class="form-select form-input-group-addon" bind:value={weightUnit}>
...
{:else}
	...
	<div class="error-state">
		<div class="error-state-content">
			...
			<p class="error-state-message">The variant you're looking for doesn't exist or has been removed.</p>
			<div class="error-state-actions">
